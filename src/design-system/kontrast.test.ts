/**
 * Lesbarkeit der Palette — nachgerechnet, nicht geschätzt.
 *
 * ─── Warum als Test und nicht als Notiz im Kommentar ───────────────────────
 *
 * Beim Übernehmen des Redesigns lagen drei der gelieferten Farbwerte unter der
 * Lesbarkeitsgrenze. Ich habe sie angehoben und die Begründung in tokens.css
 * geschrieben. Ein Kommentar hält aber niemanden davon ab, den Wert beim
 * nächsten Entwurf zurückzudrehen — dieser Test tut es.
 *
 ...
 * am schwersten. Gegen die Grundfläche allein sah `text-faint` mit 3,00 gerade
 * noch gut aus und lag auf der Karte bei 2,77.
 *
 * Grenze: 4,5 für gewöhnlichen Text, 3,0 für Text ab 24 px (bzw. 18,7 px fett)
 * sowie für Symbole und Umrisse. Die kleinen Mono-Versalien sind 9 px — also
 * 4,5, obwohl sie fett sind; deshalb nehmen sie `text-muted` und nicht
 * `text-faint` (siehe tokens.css).
 */
...
const TOKENS = readFileSync(join(process.cwd(), 'src/design-system/tokens.css'), 'utf8');
 ...
 */
/**
 * Trennstelle der beiden Sätze — am SELEKTOR, nicht an der Zeichenfolge.
 *
 * Hier stand `indexOf('[data-theme="light"]')`. Das brach in dem Moment, in
 * dem jemand denselben Text in einen Kommentar schrieb (ich, beim Berichtigen
 * des Dateikopfes): die Trennung landete bei Zeichen 845 mitten im Kommentar,
 * der "dunkle" Teil war leer, und jede Farbabfrage lief ins Leere.
 *
 * Ein Selektor steht am Zeilenanfang, ein Kommentartext praktisch nie. Das
 * `m`-Kennzeichen mit `^` trifft deshalb genau das Gemeinte.
 */
const HELL_AB = /^\[data-theme="light"\]/m.exec(TOKENS)?.index ?? -1;
const DUNKEL_TEIL = HELL_AB > 0 ? TOKENS.slice(0, HELL_AB) : TOKENS;
const HELL_TEIL = HELL_AB > 0 ? TOKENS.slice(HELL_AB) : '';

/** Liest ein HSL-Tripel aus dem gewünschten Satz und rechnet es in RGB um. */
function token(name: string, satz: 'dunkel' | 'hell' = 'dunkel'): Rgb {
    const quelle = satz === 'hell' ? HELL_TEIL : DUNKEL_TEIL;
    const m = new RegExp(`--${name}:\\s*([\\d.]+)\\s+([\\d.]+)%\\s+([\\d.]+)%`).exec(quelle);
    if (!m) throw new Error(`Token --${name} nicht im ${satz}en Satz gefunden`);
    return hslNachRgb(Number(m[1]), Number(m[2]) / 100, Number(m[3]) / 100);
}

function hslNachRgb(h: number, s: number, l: number): Rgb {
...
/** Oberer Rand des Kartenverlaufs — der hellste Untergrund im Dashboard. */
const KARTE = ueber(WEISS, GRUND, 0.045);
/** Begrüssungsbereich: Akzent 16 % auf der Grundfläche. */
const BEGRUESSUNG = ueber(token('accent-500'), GRUND, 0.16);
...
describe('Textstufen auf dem Kartengrund', () => {
    it.each([
        ...
        ['text-tertiary', 4.5],
        ['text-muted', 4.5],
    ])('%s erreicht %s:1', (name, soll) => {
        expect(kontrast(token(name), KARTE)).toBeGreaterThanOrEqual(soll);
    });

    /**
     * text-faint ist KEINE Textstufe, sondern die Stufe für Symbole und
     * Linien — nicht angeklickte Navigationszeichen, Pfeile in Kacheln, der
     * abgeschaltete Statuspunkt. Dafür gilt 3,0.
     *
     * Der Test hält zugleich fest, dass sie NICHT für Text taugt: wer sie
     * dort einsetzt, bekommt weniger als 4,5 und dieser Kommentar erklärt,
     * warum die Antwort nicht "Wert anheben" ist (siehe tokens.css).
     */
...
describe('Begruessungsbereich', () => {
    /**
     * Der hellste Punkt des hellen Verlaufs. Er steht in --hero-verlauf als
     * erster Halt; hier ausgeschrieben, weil ein Verlauf aus einem Token nicht
     * sinnvoll zu zerlegen ist und der Wert sich selten ändert.
     */
    const BEGRUESSUNG_HELL = hslNachRgb(226, 0.84, 0.94);

    it('die Mono-Datumszeile ist im Dunkeln lesbar', () => {
        expect(kontrast(token('accent-500'), BEGRUESSUNG)).toBeGreaterThanOrEqual(4.5);
    });

    it('die Mono-Datumszeile ist im Hellen lesbar', () => {
        /**
         * Hier stand accent-400, und im Dunkeln war das mit 7,77 richtig gut.
         * Im Hellen ist accent-400 aber ein MITTLERES Blau auf einem sehr
         * hellen Verlauf — 3,39, also unlesbar. accent-500 trägt beides
         * (5,28 / 5,73), deshalb steht in der Zeile jetzt eine Stufe, die in
         * beiden Sätzen funktioniert, statt zwei Stufen je Modus.
         */
        expect(kontrast(token('accent-500', 'hell'), BEGRUESSUNG_HELL)).toBeGreaterThanOrEqual(4.5);
        expect(
            kontrast(token('accent-400', 'hell'), BEGRUESSUNG_HELL),
            'accent-400 traegt im Hellen NICHT — das ist der Grund fuer accent-500',
        ).toBeLessThan(4.5);
    });

    it('der Lagesatz in text-secondary ist in beiden Saetzen lesbar', () => {
        expect(kontrast(token('text-secondary'), BEGRUESSUNG)).toBeGreaterThanOrEqual(4.5);
        expect(kontrast(token('text-secondary', 'hell'), BEGRUESSUNG_HELL)).toBeGreaterThanOrEqual(4.5);
    });

    it('die Ueberschrift traegt in beiden Saetzen', () => {
        expect(kontrast(token('text-primary'), BEGRUESSUNG)).toBeGreaterThanOrEqual(4.5);
        expect(kontrast(token('text-primary', 'hell'), BEGRUESSUNG_HELL)).toBeGreaterThanOrEqual(4.5);
    });
});
...
describe('gefuellte Akzentflaechen', () => {
     ...
     * Weiss auf accent-500 ergibt nur 3,16. Dieser Test hält das fest — sonst
    ...
    it('Weiss auf accent-600 reicht', () => {
        expect(kontrast(WEISS, token('accent-600'))).toBeGreaterThanOrEqual(4.5);
    });

    it('Weiss auf accent-700 reicht', () => {
        expect(kontrast(WEISS, token('accent-700'))).toBeGreaterThanOrEqual(4.5);
    });
});

describe('Zustandsfarben in ihren eigenen getoenten Feldern', () => {
    it.each([
        ['success', 0.06],
        ['warning', 0.07],
        ['danger', 0.07],
    ])('%s auf eigenem Feld mit %s Deckung', (name, deckung) => {
        const feld = ueber(token(name), GRUND, deckung);
        expect(kontrast(token(name), feld)).toBeGreaterThanOrEqual(4.5);
    });
});

describe('Stufen bleiben unterscheidbar', () => {
    /**
     * Fünf Textstufen nützen nichts, wenn zwei davon gleich aussehen. Geprüft
     * wird der Abstand der Leuchtdichte, nicht die Helligkeit im HSL-Sinn —
     * letztere täuscht bei verschiedenen Farbtönen.
     */
    it('jede Stufe ist von der naechsten abgesetzt', () => {
        // text-faint gehoert bewusst dazu: als Symbolstufe muss sie sich von
        // text-muted unterscheiden, sonst sehen abgeschaltete Symbole wie
        // Beschriftungen aus. Genau daran ist der erste Versuch gescheitert
        // (1,08 Abstand), siehe tokens.css.
...
describe('Hellmodus', () => {
     ...
     *
     * Gemessen wird gegen den HELLSTEN Untergrund — hier den oberen Rand des
     * Kartenverlaufs, also 4,5 % SCHWARZ auf der Grundfläche. Im Hellen ist die
     * Auflage schwarz (`--overlay: 0 0 0`), nicht weiss.
     */
    /**
    ...
    it('die Akzentrampe wird DUNKLER, nicht heller', () => {
        // #5b8cff auf Weiss ergibt 2,3 — unlesbar. Eine gespiegelte Rampe wäre
    ...
    it('die Auflage kippt von Weiss auf Schwarz', () => {
        // Der eigentliche Trick des Hellmodus. Ohne ihn wäre er nicht möglich
        // gewesen: 96 Stellen im Quelltext setzen durchscheinendes Weiss.
        expect(DUNKEL_TEIL).toMatch(/--overlay:\s*255 255 255/);
        expect(HELL_TEIL).toMatch(/--overlay:\s*0 0 0/);
    });

    it('die Lichtquellen der Grundflaeche sind im Hellen schwaecher', () => {
        const zahl = (t: string, n: string) => Number(new RegExp(`--${n}:\\s*([\\d.]+)`).exec(t)?.[1]);
        expect(zahl(HELL_TEIL, 'schein-1')).toBeLessThan(zahl(DUNKEL_TEIL, 'schein-1'));
        expect(zahl(HELL_TEIL, 'schein-2')).toBeLessThan(zahl(DUNKEL_TEIL, 'schein-2'));
    });
});

const QUELLEN_FUER_TON = [join(process.cwd(), 'src/utils/onboardingStufen.ts')];
...
describe('Karten heben sich vom Grund ab — in BEIDEN Modi', () => {
    ...
    function karteGegenGrund(satz: 'dunkel' | 'hell'): { karte: number; grund: number } {
        ...
        if (durchsichtig) {
            ...
            karte = hslNachRgb(Number(alsHsl[1]), Number(alsHsl[2]), Number(alsHsl[3]));
        } else {
...
const ALLE_QUELLEN = alleTsx(join(process.cwd(), 'src'));
