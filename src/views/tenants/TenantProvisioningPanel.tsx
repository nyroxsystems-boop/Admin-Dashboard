        ...
        <div className="mb-6 grid items-start gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(300px,1fr)]">
            <form onSubmit={event => { event.preventDefault(); mutation.mutate(); }} className="rounded-lg border border-border bg-surface">
                ...
                <fieldset disabled={mutation.isPending} className="space-y-5 p-5">
                    ...
                    <div><p className="mb-3 text-sm font-semibold">Abnahmecheckliste · {completed}/{CHECKS.length}</p><div className="divide-y divide-border rounded-md border border-border">{CHECKS.map(check => <label key={check.key} className="flex cursor-pointer items-start gap-3 p-4"><input type="checkbox" className="mt-1 size-4 accent-blue-700" checked={draft.checks[check.key] ?? false} onChange={e => setDraft({ ...draft, checks: { ...draft.checks, [check.key]: e.target.checked } })} /><span><span className="block text-sm font-medium">{check.title}</span><span className="mt-1 block text-xs leading-relaxed text-text-muted">{check.description}</span></span></label>)}</div></div>
            ...
            <aside className="space-y-5">
                <div className="rounded-lg border border-border bg-surface p-5"><h2 className="flex items-center gap-2 text-base font-semibold">{initial.readiness.ready ? <CheckCircle2 size={18} className="text-success" /> : <AlertTriangle size={18} className="text-warning" />}Freigabereife</h2><p className="mt-2 text-sm leading-relaxed text-text-secondary">Die Voraussetzungen werden beim Speichern serverseitig geprüft. Für Prüfung und Go-live müssen alle Nachweise vollständig sein.</p>
                    {initial.readiness.blockers.length ? <ul className="mt-4 space-y-3">{initial.readiness.blockers.map((blocker, index) => <li key={`${index}:${blocker}`} className="flex items-start gap-2 text-sm text-text-secondary"><Circle size={14} className="mt-1 shrink-0 text-warning" />{blocker}</li>)}</ul> : <p className="mt-4 text-sm text-success">Alle technischen und fachlichen Voraussetzungen sind erfüllt.</p>}
                </div>
